...
export class ImportFileComponent implements OnInit {
  ...
  getMovementTypes(){
    this._movementTypeService.getMovementsTypeClient().subscribe((movementTypes:MovementTypeClient[]) => {
  ...
  getInvoiceClients(){
    this._invoiceClientService.getInvoiceClients().subscribe((invoiceClients:any) => {
    ...
    fileReader.onload = (event) => {
      ...
      workbook.SheetNames.forEach(sheet => {
        ...
        data.forEach((element:any) => {
          const findMovementType:any = this.movementTypes.find((e:MovementTypeClient) => e.name_movement.toLowerCase().trim() === element.Tipo_de_movimiento.toLowerCase().trim())
          if(!findMovementType){
          ...
          const findClient:any = this.clients.find((e:Client) => e.name.toLowerCase().trim() === element.Cliente.toLowerCase().trim())
          if(!findClient){
          ...
          const findCeco:any = this.cecos.find((e: Ceco) => e.name_short.toLowerCase().trim() === element.Ceco_corto.toLowerCase().trim())
          if(!findCeco){
          ...
          const findDivisa:any = this.divisas.find((e: Divisa) => e.abbreviation_divisa.toLowerCase().trim() === element.Divisa.toLowerCase().trim())
          if(!findDivisa){
        ...
        if(this.validateExcel){
          this.history.push({
           ...
           data.forEach((element:any) => {
            ...
            this._invoiceClientService.createInvoiceClient(datos).subscribe((resp:any) => {})
        });
        }
