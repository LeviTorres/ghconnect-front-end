...
export class SearchService {
  ...
  private generateInvoiceProviders(
    invoiceProviders: any[]
  ): InvoiceProviders[] {
    return invoiceProviders.map(
      (invoiceProviders: InvoiceProviders) =>
        new InvoiceProviders(
          ...
          invoiceProviders.activities,
          invoiceProviders.followers,
    ...
    return invoiceClients.map(
      (invoiceClients: InvoiceClient) =>
        new InvoiceClient(
          ...
          invoiceClients.activities,
          invoiceClients.followers,
          invoiceClients.user,
  ...
  private generateMovementTypesClient(movements: any[]): MovementTypeClient[] {
    return movements.map(
      (movements) =>
        new MovementTypeClient(
          movements.name_movement,
          movements.key_movement,
          movements.type,
          movements.invoice,
          movements.status,
          movements._id
  ...
  private generateMovementTypesProvider(
    movements: any[]
  ): MovementTypeProvider[] {
    return movements.map(
      (movements) =>
        new MovementTypeProvider(
          movements.name_movement,
          movements.key_movement,
          movements.type,
          movements.invoice,
      ...
      .pipe(
        map((resp: any) => {
          switch (type) {
            ...
            case 'movementTypesClient':
              return this.generateMovementTypesClient(resp.resultados);
            case 'movementTypesProvider':
              return this.generateMovementTypesProvider(resp.resultados);
            default:
              return [];
          }
        })
